...
int main(){
	...
	//####################Your Tests#####################//
	cerr << "\n\tTEST #9: My Test 1 - Check playRound base case" << endl;
        /* Test to see what happens when playRound() is called without having any players
         * sitting at the table. The expected outcome is that playRound() will return an
         * empty Player vector right at the beginning. */

        Table ttable(5, ante);
        vector<Player> tlosers = ttable.playRound();
        assert(tlosers.size() == 0);

        cerr << "\n\t========================PASS========================\n" << endl;

        cerr << "\n\tTEST #10: My Test 2 - Attempt to add player with low budget" <<endl;
        /* Test to see what happens when a player with a budget lower than the required ante
         * is added to the table. The expected outcome is that the player is not added to the
         * table because he does not meet the budget requirement. */

        Player tplayer = Player("Mark",1);
        Table ttable2 = Table(1,2);
        ttable.addPlayer(tplayer);
        assert(ttable2.emptySeat());
